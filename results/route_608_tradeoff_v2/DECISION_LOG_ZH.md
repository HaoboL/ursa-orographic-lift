...
## D018：方法文字必须服从实际提交实现，而不是历史目标分配叙述

最终公式—实现合同复核最初失败。逐项检查确认，数值实现始终使用以下合同：源山高度为峰顶减背风基脚，目标山高度为峰顶减迎风基脚；受支持配对要求横风足迹重叠、正确顺风次序及 `0 < D_st <= 19.2 H_s`；每个目标选最近的受支持上游源；外层响应作用于目标迎风面；最终保持率从基础 `S0` 和覆盖该点的外层响应中取最小。正文/Supplement 的旧句却声称目标块互不重叠、不取跨目标最小值且不设置 `19.2H` 距离门，和实际执行代码不一致。

决定：不改已经生成正式结果的科学代码或冻结系数，只把英文、中文及 Supplement 修正为实际实现：配对外保持 `S=S0`，重叠迎风面取最小保持率，`19.2H_s` 是受 Karim 等报告恢复尺度启发的冻结外层边界而非普适常数。公式—实现审计同步到当前 1.30/0.25 合同，并检查两种基脚、高度语义、配对域、迎风面、最小合并、5 m AGL、联合 false-lift 概率和 terrain-macro 聚合；全部门通过。该问题分类为 agent-created manuscript/contract drift，数值结果不变，但旧方法文字不得提交。
